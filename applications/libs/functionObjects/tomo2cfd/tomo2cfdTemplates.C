...
template<class Type>
bool Foam::functionObjects::tomo2cfd::foundObject
(
    const word& name,
    const bool verbose
) const
...
template<class Type>
void Foam::functionObjects::tomo2cfd::interpolateFields
(
...
)
{
    ...
    if (obr_.foundObject<fieldType>(fieldName))
    {
        ...
        for( int i0; i0<dimensions_[0]; i0++)
        {
            for( int i1; i1<dimensions_[1]; i1++)
            {
                for( int i2; i2<dimensions_[2]; i2++)
                {
