...
public:
  ...
  ( // (mesh, faceDisp, patchID, majDir, minDir)
  ...
  // void getSurfaceDisplacement  // calculations are here
  // (
  //   dynamicFvMesh& mesh,
  //   scalarField& wd,
  //   label& patchID,
...
int main(int argc, char *argv[])
{
  ...
  IOdictionary surfRoughGenDict // generate dictionary data class object
  (
    IOobject
    (
      "surfRoughGenDict",
...
///////
  // Read interpolation (only done once typically in createFields.h)
  dictionary FDict;
  IOobject FDictIO
  (
    "F", // name of the file
  ...
  FDict = IOdictionary(FDictIO);
  // Create a function to interpolate F, the function is defined in the case settings
  autoPtr<Function1<vector>> function_ 
  (
      Function1<vector>::New("function", FDict)
  );
  scalarField pointDisp2(19398, 0.0); //TODO: generalize the size of vector
  int i = 0;
  int iter = 0;
  ...
  for(int m=0; m<M; ++m)
  {
    for(int n=0; n<N; ++n)
    {
      i += 1;
      iter = 282 * n + m;
      pointDisp2[i] = function_->value(iter)[2];
      Info << iter << endl;
    }
  }
  ...
  scalarField faceDisp(pointNface.size(), 0.001);
  // if I input 0.001 here, then 0 on the bottom side will be - 0.001
  // rg.getSurfaceDisplacement(mesh, faceDisp, patchID, majDir, minDir);  //here
